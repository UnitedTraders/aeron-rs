...
impl BroadcastReceiver {
    pub fn new(buffer: AtomicBuffer) -> Result<BroadcastReceiver, BroadcastTransmitError> {
        ...
        let m_cursor = rx.buffer.get::<i64>(rx.latest_counter_index);
        let next_record = rx.cursor;

        rx.record_offset = (rx.cursor & rx.mask as i64) as i32;
    ...
    pub fn receive_next(&mut self) -> bool {
        ...
        if tail > cursor {
            let mut record_offset: Index = (cursor & self.mask as i64) as i32;

            if !self.do_validate(cursor as i32) {
//                self.m_lappedCount += 1;
                cursor = self.buffer.get::<i64>(self.latest_counter_index);
                record_offset = (cursor & self.mask as i64) as i32;
            }

            self.cursor = cursor;
            self.next_record = cursor + align(
                self.buffer.get::<i32>(
                    record_descriptor::length_offset(record_offset)), record_descriptor::RECORD_ALIGNMENT) as i64;
            ...
            if record_descriptor::PADDING_MSG_TYPE_ID == self.buffer.get::<i32>(record_descriptor::type_offset(record_offset))
            {
                ...
                self.next_record += align(self.buffer.get::<i32>(
                    record_descriptor::length_offset(record_offset)), record_descriptor::RECORD_ALIGNMENT) as i64;
            }
    ...
    pub fn validate(&self) -> bool {
        atomics::acquire();
        self.do_validate(self.cursor as i32)
    }

    fn do_validate(&self, cursor: Index) -> bool {
        cursor + self.capacity > self.buffer.get_volatile::<i64>(self.tail_intent_counter_index) as i32
    }
}
...
mod tests {
    ...
    fn channel(capacity: usize) -> (BroadcastTransmitter, BroadcastReceiver) {
        ...
        (BroadcastTransmitter::new(buffer).unwrap(), BroadcastReceiver::new(buffer).unwrap())
    }
