...
impl CopyBroadcastReceiver {
    pub fn new(receiver: BroadcastReceiver,
               scratch_buffer: AtomicBuffer) -> Self {
        Self {
            receiver,
            scratch_buffer,
        }
    ...
    pub fn receive<F>(&mut self, handler: F) -> Result<usize, BroadcastTransmitError>
        ...
        if self.receiver.receive_next() {
            ...
            if length > self.scratch_buffer.capacity() {
                return Err(BroadcastTransmitError::BufferTooSmall { need: length, capacity: self.scratch_buffer.capacity() });
            }
